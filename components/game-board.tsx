      ...
      <div className="grid grid-cols-8 gap-1.5">
        {board.map((row, rowIndex) =>
          row.map((cell, colIndex) => {
            ...
            const cellBgColor = cell ? colorMap[cell] || "bg-gray-500" : "bg-gray-700"
            const previewBgColor = previewInfo.isValid ? colorMap[previewInfo.color] || "bg-gray-500" : "bg-red-500"

            return (
              <motion.div
                key={`${rowIndex}-${colIndex}`}
                className={`
                  w-10 h-10 md:w-11 md:h-11 rounded-sm relative cursor-pointer
                  ${cellBgColor}
              ...
              >
                ...
                {previewInfo.isPreview && (
                  <div
                    className={`
                      absolute inset-0 rounded-sm opacity-60
                      ${previewBgColor}
                    `}
                  />
                )}
